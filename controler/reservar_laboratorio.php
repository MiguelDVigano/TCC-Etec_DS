...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($idx_inicio === -1 || $idx_fim === -1 || $idx_inicio > $idx_fim) {
        header("Location: ../view/reserva_de_labs.php?erro_reserva=" . urlencode("Horário inválido selecionado."));
        exit;
    }
    ...
    if ($conflict) {
        header("Location: ../view/reserva_de_labs.php?erro_reserva=" . urlencode("Erro: Um ou mais horários já estão reservados."));
        exit;
    } else {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ../view/reserva_de_labs.php?reserva=ok");
            exit;
        } else {
